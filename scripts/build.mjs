...
function renderHome(homePosts, tags) {
  ...
  return `
    ...
    <section class="section-heading" aria-labelledby="latest-posts">
      <div>
        <h2 id="latest-posts">Latest notes</h2>
        <p>Builder logs, life notes, references, and things worth remembering.</p>
      </div>
    </section>
...
function renderPost(post) {
  return `
    <article class="article">
      ...
      <a class="back-link" href="${withBase("/")}">Back to all notes</a>
    </article>
  `;
}
...
function renderTagPage(tag, taggedPosts) {
  return `
    <section class="hero">
      ...
      <p>${taggedPosts.length} ${taggedPosts.length === 1 ? "note" : "notes"} saved here.</p>
    </section>
    <div class="post-list">
...
function renderMeta(post) {
  return `
    <p class="post-meta">
      ...
      <span>${post.tags.length ? `${post.tags.length} tags` : "No tags"}</span>
    </p>
  `;
}
    ...
    <div class="site-shell">
      <header class="site-header">
        ...
        <nav class="nav" aria-label="Primary navigation">
          ...
          ${config.social.github ? `<a href="${escapeHtml(config.social.github)}">GitHub</a>` : ""}
          ${config.social.x ? `<a href="${escapeHtml(config.social.x)}">X</a>` : ""}
        </nav>
      </header>
      <main>
        ${body}
      ...
      <footer class="site-footer">
        <span>Built as a small public notebook. Last generated ${new Date().toISOString().slice(0, 10)}.</span>
      </footer>
    </div>
  </body>
</html>`;
